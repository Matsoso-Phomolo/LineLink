...
def complaint_in_scope(
    ...
    if user.role == UserRole.tenant:
        if (
            complaint.sender_user_id != user.id
            and complaint.receiver_user_id != user.id
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Complaint is outside your scope",
            )

        return complaint

    scoped_complaint = (
        scoped_query(db, user, Complaint)
        .filter(Complaint.id == complaint.id)
        .first()
    )

    if not scoped_complaint:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Complaint is outside your scope",
        )
...
def list_complaints(
    ...
    if user.role == UserRole.tenant:
        return (
            ...
            .filter(
                (Complaint.sender_user_id == user.id)
                | (Complaint.receiver_user_id == user.id)
            )
...
def update_complaint(
    ...
    complaint = complaint_in_scope(db, user, complaint_id)
