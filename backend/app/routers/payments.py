...
def find_transaction_from_dict(db: Session, payload: dict) -> PaymentTransaction | None:
    event_id = payload.get("webhook_event_id") or payload.get("event_id") or payload.get("id")
    if event_id:
        item = db.query(PaymentTransaction).filter(PaymentTransaction.webhook_event_id == str(event_id)).first()
        if item:
            return item
    normalized = PaymentCallbackPayload(
        checkout_request_id=payload.get("checkout_request_id") or payload.get("checkoutRequestId"),
        provider_reference=payload.get("provider_reference") or payload.get("providerReference") or payload.get("reference"),
        idempotency_key=payload.get("idempotency_key") or payload.get("idempotencyKey"),
        status=str(payload.get("status") or payload.get("provider_status") or payload.get("providerStatus") or ""),
        amount=payload.get("amount"),
        transaction_reference=payload.get("transaction_reference") or payload.get("transactionReference"),
        message=payload.get("message"),
        error_message=payload.get("error_message") or payload.get("failure_reason") or payload.get("failureReason"),
    )
...
def complete_successful_subscription_transaction(db: Session, transaction: PaymentTransaction, payload: PaymentCallbackPayload) -> None:
    subscription = db.get(LandlordSubscription, transaction.subscription_id) if transaction.subscription_id else None
    landlord = db.get(Landlord, transaction.landlord_id)
    if not subscription or not landlord:
        transaction.failure_reason = "Subscription or landlord was not found for successful payment"
        return
    ...
    if subscription.renewal_date and subscription.renewal_date > today:
        ...
        subscription.renewal_date = subscription.renewal_date.replace(year=year, month=month)
    else:
        month = today.month + 1
    ...
    if landlord.user_id:
        db.add(Notification(user_id=landlord.user_id, title="Subscription payment successful", body="Your LineLink landlord subscription payment was confirmed.", category="subscriptions"))
    receipt = PaymentReceipt(
        landlord_id=transaction.landlord_id,
        tenant_id=None,
        room_id=None,
...
def initiate_payment(payload: PaymentInitiateRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    if not payload.rent_due_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="rent_due_id is required for tenant rent payments")
...
def get_transaction(transaction_id: uuid.UUID, db: Session = Depends(get_db), user: User = Depends(require_roles(UserRole.admin, UserRole.landlord, UserRole.caretaker, UserRole.tenant))):
    ...
    if user.role == UserRole.tenant:
        ...
        if not tenant or transaction.tenant_id != tenant.id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Payment transaction is outside your account")
    else:
        if not landlord_scope_filter(db, user, PaymentTransaction).filter(PaymentTransaction.id == transaction.id).first():
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Payment transaction is outside your scope")
    return transaction


@router.get("/receipts")
def list_receipts(db: Session = Depends(get_db), user: User = Depends(require_roles(UserRole.admin, UserRole.landlord, UserRole.caretaker, UserRole.tenant))):
    ...
    if user.role == UserRole.tenant:
        ...
        scoped = landlord_scope_filter(db, user, PaymentReceipt)
        query = scoped
    return query.order_by(PaymentReceipt.issued_at.desc()).limit(100).all()


@router.get("/gateway-health")
def gateway_health(db: Session = Depends(get_db), _: User = Depends(require_roles(UserRole.admin))):
    last = db.query(PaymentTransaction).filter(PaymentTransaction.webhook_event_id.is_not(None)).order_by(PaymentTransaction.processed_at.desc().nullslast(), PaymentTransaction.updated_at.desc()).first()
    return {
        "mopay_environment": settings.mopay_environment,
        "webhook_url": settings.mopay_callback_url or f"{settings.public_base_url}/payments/callback/mopay",
        ...
        "failed_webhook_count": db.query(PaymentTransaction).filter(PaymentTransaction.payment_type.in_(["rent", "landlord_subscription"]), PaymentTransaction.status == PaymentTransactionStatus.failed).count(),
        "successful_payment_count": db.query(PaymentTransaction).filter(PaymentTransaction.status == PaymentTransactionStatus.successful).count(),
    }
...
async def mopay_callback(request: Request, db: Session = Depends(get_db)):
    raw_body = await request.body()
    signature = request.headers.get("x-mopay-signature") or request.headers.get("x-signature")
    verified = verify_mopay_signature(raw_body, signature)
    if settings.mopay_webhook_secret and not verified:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid MoPay webhook signature")
    try:
    ...
    if event_id and transaction.webhook_event_id == str(event_id) and transaction.processed_at:
        return {"detail": "duplicate webhook ignored", "transaction_id": str(transaction.id), "status": transaction.status.value}
    callback = PaymentCallbackPayload(
        checkout_request_id=payload.get("checkout_request_id") or payload.get("checkoutRequestId") or transaction.checkout_request_id,
        provider_reference=payload.get("provider_reference") or payload.get("providerReference") or payload.get("reference") or transaction.provider_reference,
        idempotency_key=payload.get("idempotency_key") or payload.get("idempotencyKey") or transaction.idempotency_key,
        status=str(payload.get("status") or payload.get("provider_status") or payload.get("providerStatus") or ""),
        amount=payload.get("amount") or float(transaction.amount),
        transaction_reference=payload.get("transaction_reference") or payload.get("transactionReference") or payload.get("receipt_number"),
        message=payload.get("message"),
        error_message=payload.get("error_message") or payload.get("failure_reason") or payload.get("failureReason"),
    )
    ...
    if normalized_status in {"success", "successful", "paid", "completed"}:
        ...
        transaction.provider_error = callback.error_message or callback.message or "MoPay reported payment failure"
...
def verify_mopay_signature(raw_body: bytes, signature: str | None) -> bool:
    if not settings.mopay_webhook_secret:
        return False
    if not signature:
        return False
    expected = hmac.new(settings.mopay_webhook_secret.encode("utf-8"), raw_body, hashlib.sha256).hexdigest()
