...
def approve_landlord_verification(
    ...
    payload: LandlordRequestDecision,
    db: Session = Depends(get_db),
    admin: User = Depends(require_roles(UserRole.district_admin)),
):
    request = db.get(LandlordRequest, request_id)
    ...
    password = payload.password or first_name_password(request.full_name)
...
def disable_landlord(
    ...
    assert_landlord_access(db, current_user, landlord.id)
